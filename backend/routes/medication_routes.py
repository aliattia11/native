...
@medication_routes.route('/api/medication-schedules/<patient_id>', methods=['GET'])
@token_required
@api_error_handler
...
def create_or_update_schedule(current_user, patient_id):
    try:
        ...
        logger.info(f"Received schedule update request: {request.json}")

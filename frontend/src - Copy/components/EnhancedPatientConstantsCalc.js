...
// Calculation functions remain the same as they don't depend on context
...
export const calculateActivityImpact = (activities, constants) => {
  return activities.reduce((total, activity) => {
    const coefficient = constants.activity_coefficients[activity.level] || 0;
...
// Custom hook to use constants with calculations
export const useConstantsCalculations = () => {
  ...
  const calculateInsulinDose = ({
    ...
    // Use patient constants from context with fallback to defaults
    ...
    if (bloodSugar) {
      const glucoseDifference = bloodSugar - constants.target_glucose;
      correctionInsulin = glucoseDifference / constants.correction_factor;
    }

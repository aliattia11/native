...
import { useConstantsCalculations } from './EnhancedPatientConstantsCalc';
import axios from 'axios';
import { FaPlus, FaMinus } from 'react-icons/fa';
import DurationInput from './DurationInput';
import FoodSection from './FoodSection';
import { calculateTotalNutrients } from './EnhancedPatientConstantsCalc';
...
const MealInput = () => {
  ...
  const { calculateInsulinDose } = useConstantsCalculations();
  ...
  const calculateInsulinNeeds = useCallback(() => {
    ...
    try {
      ...
      const insulinCalculation = calculateInsulinDose({
        ...
        activities,
        absorptionType: totalNutrition.absorptionType
      });
  ...
  }, [selectedFoods, bloodSugar, activities, calculateInsulinDose, patientConstants]);

  useEffect(() => {
    if (!loading && (selectedFoods.length > 0 || activities.length > 0 || bloodSugar)) {
      calculateInsulinNeeds();
    }
  }, [selectedFoods, activities, bloodSugar, mealType, loading, calculateInsulinNeeds]);
  ...
  if (error) {
    return <div>Error loading patient constants: {error}</div>;
  }

...
export function ConstantsProvider({ children }) {
  ...
  try {
    ...
    if (!constants) {
      ...
      // If we have a patient ID, fetch their medication schedules
      if (constants.patient_id) {
        try {
          const schedulesResponse = await axios.get(
            `http://localhost:5000/api/medication-schedule/${constants.patient_id}`,
            {
              headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
          ...
          if (schedulesResponse.data.schedules) {
            const schedulesMap = schedulesResponse.data.schedules.reduce((acc, schedule) => {
              acc[schedule.medication] = schedule;
              return acc;
            }, {});

            // Update medication schedules in constants
            constants.medication_schedules = schedulesMap;
          }
        } catch (scheduleError) {
          console.error('Error fetching medication schedules:', scheduleError);
        }
      }
    ...
    setError(`Failed to load patient constants: ${errorMessage}`);
    setPatientConstants(DEFAULT_PATIENT_CONSTANTS);
  } finally {
    setLoading(false);
  }
